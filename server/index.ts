...
function serveExpoManifest(platform: string, req: Request, res: Response) {
  ...
  if (manifest.assets) {
    for (const asset of manifest.assets) {
      if (asset.url) {
        asset.url = asset.url.replace(/https?:\/\/[^/]+/, actualBaseUrl);
      }
    }
  }
  ...
  if (client) {
    if (client.iconUrl) {
      client.iconUrl = client.iconUrl.replace(/https?:\/\/[^/]+/, actualBaseUrl);
    }
    if (client.hostUri) {
      client.hostUri = `${host}/${platform}`;
    }
    if (client.android?.adaptiveIcon?.foregroundImageUrl) {
      client.android.adaptiveIcon.foregroundImageUrl =
        client.android.adaptiveIcon.foregroundImageUrl.replace(
          /https?:\/\/[^/]+/,
          actualBaseUrl,
        );
    }
  ...
  if (expoGo) {
    if (expoGo.debuggerHost) {
      expoGo.debuggerHost = `${host}/${platform}`;
    }
  }
...
function serveLandingPage({
  ...
  const html = landingPageTemplate
    ...
    .replace(/EXPO_PROJECT_ID/g, "7a60693a-e5f2-4a7f-8760-5d328090cfd2");
